    ...
    public boolean keyDown(int keycode) {
        ...
        switch (map.currentMode){
            ...
            case Auction:
                break;
            case Store:
            	System.out.println("Anddd I'm in the store nigga");
                break;
            case Pub:
                break;
            case MuleLand:
                ...
                break;
            case Town:
            	System.out.println("(" + (map.getCurrentPlayer().getX()) + "," + (map.getCurrentPlayer().getY()) + ")" );
                ...
                //Code for going to the store (Assay)
                if(map.getCurrentPlayer().getX()>=143 && map.getCurrentPlayer().getX()<=203 && map.getCurrentPlayer().getY() >= 334
                        &&map.getCurrentPlayer().getY()<=384){
                	System.out.println("Player has entered the Store!");
                	map.store();
                }
                
                if(map.getCurrentPlayer().getX()>=523 && map.getCurrentPlayer().getX()<=603 && map.getCurrentPlayer().getY() >= 334
                        &&map.getCurrentPlayer().getY()<=384){	
                	map.muleShop();
                }
